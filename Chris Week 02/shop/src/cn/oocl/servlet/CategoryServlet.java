...
//		System.out.println("request data" + request.getParameter("type") + " " + request.getParameter("hot"));
		...
		if(status.equals("save")){
			...
			RequestDispatcher requestDispactcher = request.getRequestDispatcher("/categoryUpdate.jsp"); //no need include project name, because 
			...
			request.setAttribute("categoryList", categoryList);
			RequestDispatcher requestDispactcher = request.getRequestDispatcher("/categoryUpdate.jsp"); 
			requestDispactcher.forward(request, response);
		}
			...
			// 3: get the result from Service then send back to UI
	        response.sendRedirect("/shop/categoryUpdate.jsp");
		}
	}
}
